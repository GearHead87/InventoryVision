...
export const useGetCategoryOption = (): { categoryOption: TCategoryOption[] } => {
	const [categoryOption, setCategoryOption] = useState<TCategoryOption[]>([]);

	useEffect(() => {
		const options = categories.map((category: TCategory) => ({
			value: category.slug,
			label: category.name,
		}));
